...
from telegram import Update, ReplyKeyboardMarkup, ReplyKeyboardRemove, KeyboardButton, WebAppInfo, InlineKeyboardMarkup, InlineKeyboardButton, BotCommand, BotCommandScopeChat, InputFile
...
async def init_db(app) -> None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS user_waters (
            ...
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS user_attraction_marks (
                    user_id   BIGINT NOT NULL,
                    key       TEXT   NOT NULL,
                    marked_at TIMESTAMP DEFAULT NOW(),
...
def _populate_waters_if_empty(conn) -> None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            pid = ocean_ids["Тихий океан"]
            atl = ocean_ids["Атлантический океан"]
            ind = ocean_ids["Индийский океан"]
...
async def record_user(user_id: int, username: str | None, first_name: str | None) -> None:
    """Сохраняет/обновляет пользователя. При первом обращении автоматически
    активирует 7-дневный премиум-триал (если ещё не был использован)."""
    try:
        conn = get_db_connection()
        try:
            with conn.cursor() as cur:
...
def is_premium(user_id: int) -> bool:
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT is_premium, premium_expires_at FROM users WHERE user_id = %s",
                    (user_id,),
                )
                row = cur.fetchone()
                if not row:
                    return False
                db_is_premium, expires_at = row
...
MAIN_MENU, ANSWERING, HELP_MENU, HELP_TOPIC, TRANSLATING, VISA_MENU, VISA_CATEGORY, \
    ...
    FEEDBACK_TYPING, \
    WHERE_TO_FLY, WHERE_TO_FLY_MONTH = range(47)
...
def get_main_keyboard():
    return ReplyKeyboardMarkup(
        [
            ...
            [KeyboardButton("🗺 Мои путешествия"), KeyboardButton("⭐ Премиум")],
            [KeyboardButton("🎮 Игры"),             KeyboardButton("🤝 Партнёры")],
            [KeyboardButton("👤 О проекте"),        KeyboardButton("🆘 Поддержка")],
    ...
    return ReplyKeyboardMarkup(
        [
            ...
            [gated("💰 Общий счёт",                           SPLITWISE_URL),
             KeyboardButton("📚 Путеводители")],
            [KeyboardButton("✈️ Авторские туры"),             KeyboardButton("🛃 Оформить визу")],
            [KeyboardButton("◀️ Назад"),                      KeyboardButton(HOME_BTN)],
        ],
        resize_keyboard=True,
        one_time_keyboard=True,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🗺 Мои страны · Статистика\n"
        "🌤 Погода · Переводчик · Конвертер валют\n"
        "🛫 Лаунджи · Несовместимые страны\n"
        "🎮 Игры · Викторина · Страна дня\n"
        "✅ Дневник путешественника · Чеклист\n\n"
        "🤝 Официальные партнёры:\n"
        "Aviasales · Tripster · Cherehapa · Airalo и другие\n\n"
        "Бот бесплатный — только часть функций по подписке.\n"
        "🎁 Тебе активированы 7 дней полного доступа бесплатно!\n\n"
        "Выбери раздел 👇",
        reply_markup=get_main_keyboard(),
    )
    return MAIN_MENU
...
async def main_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Feature buttons ──────────────────────────────────────────────────────
        ...
        with open('kirill.jpg', 'rb') as f:
            await update.message.reply_photo(InputFile(f))
        await update.message.reply_text(
            "👤 О проекте\n\n"
            "Привет, я Кирилл Андреев — основатель «Как местный».\n\n"
            "В прошлом году я ушёл с нелюбимой работы и завязал с нелюбимым бизнесом. Решил заниматься только тем, от чего горят глаза. Путешествия — именно это.\n\n"
            "🌍 Немного цифр:\n"
            "— 42 страны, 200+ городов\n"
            "— Самое долгое путешествие — 5 месяцев\n"
            "— 8 стран, 13 городов за одну поездку\n"
            "— 20 стран за один год\n"
            "— 3 месяца жил на Кипре\n"
            "— Путешествую только с ручной кладью\n"
            "— Все страны — без знания английского\n\n"
            "Большинство поездок соло. Никаких отельных туров — только самостоятельно спланированные путешествия. Бывало всякое — из любой ситуации всегда находил выход.\n\n"
            "✈️ Что я делаю:\n"
            "Создаю бизнесы в теме путешествий. Пишу авторские путеводители. Составляю туры в любую страну мира — могу разработать маршрут, а могу сопроводить лично. Всё что связано с путешествиями — меня триггерит, и я хочу быть компетентен в каждом вопросе.\n\n"
            "🎬 Скоро запускаю YouTube канал — будет честно и без прикрас о путешествиях и жизни.\n\n"
...
async def show_tours_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Лето / осень 2026 · 5 дней\n\n"
        "💰 ~70 000 ₽ с человека\n"
        "Всё включено: трансфер, проживание, питание, гиды, активности.\n"
        "Не включено: авиабилеты и карманные расходы.\n\n"
        "⚠️ Это проверка спроса — никаких обязательств и предоплаты. Тур состоится если будет достаточный отклик.\n\n"
        "👇 Нажми кнопку «Хочу в тур» ниже — и я лично напишу тебе с подробностями о программе и датах.",
        parse_mode="Markdown",
...
EVENTS_SECTIONS = {
    "🏅 Спортивные события": (
        ...
        "⚠️ _Информация актуальна на момент публикации. Перед поездкой уточняйте даты и детали на официальных сайтах мероприятий._"
    ),
    ...
    "🎭 Праздники и культура": (
        ...
        "🗓 Январь (ежегодно)\n"
        "👥 5 млн+ посетителей\n"
        "✨ Скидки до 75%, розыгрыши авто, шоу и развлечения\n\n"
        "🎆 *Китайский Новый год*\n"
        ...
        "🏮 *Фестиваль фонарей*\n"
        "📍 Тайвань / Таиланд (Йи Пэн) 🇹🇼🇹🇭\n"
        "🗓 Февраль (полнолуние)\n"
        "👥 Сотни тысяч участников\n"
        "✨ Тысячи светящихся фонариков в небе — незабываемое зрелище\n\n"
        "— *МАРТ–АПРЕЛЬ* —\n\n"
        "🎉 *Карнавал в Рио*\n"
        ...
        "🌊 *Сонгкран — Новый год воды*\n"
        "📍 Таиланд 🇹🇭 (Чиангмай, Бангкок)\n"
        "🗓 13–15 апреля ежегодно\n"
        "👥 Миллионы участников по всей стране\n"
        "✨ Крупнейшее водное сражение в мире — улицы превращаются в реки\n\n"
        "— *МАЙ–ИЮНЬ* —\n\n"
        "🐂 *Коррида Сан-Исидро*\n"
        "📍 Мадрид, Испания 🇪🇸\n"
        "🗓 Май (18 дней)\n"
        ...
        "☀️ *Инти Райми — Праздник Солнца*\n"
        "📍 Куско, Перу 🇵🇪\n"
        "🗓 24 июня ежегодно\n"
        "👥 100 000+ зрителей\n"
        "✨ Древний ритуал инков у крепости Саксайуаман — театр под открытым небом\n\n"
        "— *ИЮЛЬ–АВГУСТ* —\n\n"
        ...
        "📍 Париж, Франция 🇫🇷\n"
        "🗓 14 июля ежегодно\n"
        "👥 500 000+ на Елисейских полях\n"
        "✨ Военный парад, фейерверк у Эйфелевой башни — главный праздник Франции\n\n"
        "🍅 *Томатина*\n"
        "📍 Буньоль, Испания 🇪🇸\n"
        ...
        "🗓 1–2 ноября\n"
        "👥 Миллионы по всей стране\n"
        "✨ Алтари, цветы мексиканского ноготка, грим Катрины — красочный и глубокий праздник\n\n"
        "❄️ *Харбинский фестиваль льда*\n"
        "📍 Харбин, Китай 🇨🇳\n"
        "🗓 Декабрь–февраль\n"
        "👥 1 млн+ посетителей\n"
        "✨ Ледяные дворцы высотой 30 м, ледяные горки, световые шоу\n\n"
        "🎆 *Новый год в Сиднее*\n"
        "📍 Сидней, Австралия 🇦🇺\n"
        "🗓 31 декабря\n"
    ...
    "👗 Мода и стиль": (
        "👗 *МОДА И СТИЛЬ 2026*\n\n"
        "👗 Нью-Йорк Fashion Week — Нью-Йорк, США | 11–16 февраля 2026\n"
        "👗 Лондон Fashion Week — Лондон, Великобритания | 19–23 февраля 2026\n"
        "👗 Милан Fashion Week — Милан, Италия | 24 февраля – 2 марта 2026\n"
        "👗 Париж Fashion Week — Париж, Франция | 2–10 марта 2026\n"
        "👗 Сеул Fashion Week — Сеул, Южная Корея | Март 2026\n"
        "👗 Копенгаген Fashion Week — Копенгаген, Дания | Январь/Август 2026\n"
        "👗 Милан Fashion Week (осень) — Милан, Италия | 22–28 сентября 2026\n"
        "👗 Париж Haute Couture — Париж, Франция | Июль 2026\n\n"
        "ℹ️ Даты могут меняться — уточняй на официальных сайтах перед поездкой"
    ),
    "🎬 Кино и премии": (
        "🎬 *КИНО И ПРЕМИИ 2026*\n\n"
        "🏆 Золотой глобус — Лос-Анджелес, США | 11 января 2026\n"
        "🎵 Грэмми — Лос-Анджелес, США | 1 февраля 2026\n"
        "🎬 BAFTA — Лондон, Великобритания | 22 февраля 2026\n"
        "🏆 Оскар — Лос-Анджелес, США | 15 марта 2026\n"
        "🎭 Тони (театр) — Нью-Йорк, США | Июнь 2026\n"
        "📺 Эмми — Лос-Анджелес, США | Сентябрь 2026\n"
        "🎥 Каннский кинофестиваль — Канны, Франция | 12–23 мая 2026\n"
        "🎥 Венецианский кинофестиваль — Венеция, Италия | Август–сентябрь 2026\n"
        "🎥 Берлинале — Берлин, Германия | Февраль 2026\n"
        "🎥 Сандэнс — Парк-Сити, США | Январь 2026\n"
        "🎥 TIFF — Торонто, Канада | Сентябрь 2026\n\n"
        "ℹ️ Даты могут меняться — уточняй на официальных сайтах перед поездкой"
    ),
}
...
WHERE_TO_FLY_DATA = {
    "Январь": [
        "🇹🇭 Таиланд — сухой сезон, +32°C. Пхукет, Краби, острова Самуи",
        "🇦🇪 ОАЭ — +25°C, солнечно. Дубай, Абу-Даби, пляжи Джумейры",
        "🇲🇻 Мальдивы — +30°C, штиль. Лучшее время для снорклинга",
        "🇮🇩 Бали — сухой сезон на севере, +30°C. Убуд, Семиньяк",
        "🇻🇳 Вьетнам (юг) — +30°C, солнечно. Фукуок, Муйне",
        "🇲🇽 Мексика — +28°C, сухо. Канкун, Плая-дель-Кармен, Тулум",
        "🇰🇪 Кения — +28°C, сухой сезон. Сафари в Масаи-Мара",
        "🇨🇺 Куба — +26°C, лучший сезон. Гавана, Варадеро",
        "🇧🇷 Бразилия — +30°C, карнавал в феврале. Рио, Форталеза",
        "🇴🇲 Оман — +22°C, идеально. Маскат, вади, пустыня Вахиба",
        "🇹🇿 Танзания — +28°C, сухо. Занзибар, Серенгети, Килиманджаро",
        "🇱🇰 Шри-Ланка (юг) — +30°C. Галле, Мирисса, киты",
        "🇵🇪 Перу — +24°C, сухо на побережье. Лима, Мачу-Пикчу",
        "🇵🇭 Филиппины — +30°C. Палаван, Боракай, Себу",
        "🇨🇴 Колумбия — +28°C. Картахена, Медельин, Кофейный регион",
    ],
    "Февраль": [
        "🇹🇭 Таиланд — пик сезона, +33°C. Чиангмай, острова",
        "🇦🇪 ОАЭ — +24°C, комфортно. Дубай, пустынные сафари",
        "🇲🇻 Мальдивы — +30°C, идеальная видимость под водой",
        "🇻🇳 Вьетнам (юг) — +31°C. Хошимин, Фукуок, дельта Меконга",
        "🇸🇬 Сингапур — +30°C, фестиваль китайского Нового года",
        "🇲🇽 Мексика — +28°C, сухо. Юкатан, пирамиды майя, сеноты",
        "🇨🇺 Куба — +26°C, лучший месяц. Тринидад, Варадеро",
        "🇴🇲 Оман — +23°C. Джераш, горы Хаджар, морские черепахи",
        "🇯🇴 Иордания — +18°C, сухо. Петра, Вади-Рам, Мёртвое море",
        "🇿🇦 ЮАР — +26°C, лето. Кейптаун, сафари Крюгер",
        "🇱🇰 Шри-Ланка (юг) — +31°C, пик сезона. Унаватуна, Тангалле",
        "🇵🇭 Филиппины — +30°C, лучший месяц. Эль-Нидо, Коррехидор",
        "🇹🇿 Танзания — +29°C. Занзибар, дайвинг, дельфины",
        "🇮🇳 Индия (Керала) — +32°C. Бэкуотерс, Коччи, чайные плантации",
        "🇦🇷 Аргентина — +28°C, лето. Буэнос-Айрес, Патагония",
    ],
    "Март": [
        "🇻🇳 Вьетнам (центр) — +28°C, сухо. Дананг, Хойан, Хюэ",
        "🇯🇴 Иордания — +20°C, цветение. Петра, Акаба, Вади-Рам",
        "🇲🇦 Марокко — +22°C, весна. Марракеш, Фес, Сахара",
        "🇬🇷 Греция — +18°C, безлюдно. Афины, Крит, Санторини",
        "🇵🇹 Португалия — +18°C, цветение миндаля. Лиссабон, Алгарве",
        "🇮🇳 Индия (Гоа) — +32°C, конец сезона. Пляжи, форты",
        "🇨🇾 Кипр — +20°C, цветение. Пафос, Лимасол, горы Троодос",
        "🇱🇰 Шри-Ланка — +31°C. Сигирия, Канди, Анурадхапура",
        "🇮🇳 Индия (Раджастан) — +28°C. Джайпур, Удайпур, Пушкар",
        "🇵🇭 Филиппины — +30°C. Висайские острова, вулкан Майон",
        "🇸🇬 Сингапур — +30°C. Гарденс бай зе Бей, Сентоза, Чайнатаун",
        "🇦🇪 ОАЭ — +26°C. Дубай, Шарджа, горы Хатта",
        "🇪🇬 Египет — +22°C, сухо. Каир, пирамиды, Луксор",
        "🇹🇿 Танзания — +28°C. Великая миграция начинается. Серенгети",
        "🇲🇽 Мексика — +28°C. Оахака, Мехико, пирамиды Теотиуакан",
    ],
    "Апрель": [
        "🇬🇷 Греция — +20°C, мало туристов. Санторини, Миконос",
        "🇵🇹 Португалия — +20°C, отлично. Порту, Синтра, Алентежу",
        "🇲🇦 Марокко — +25°C, идеально. Шефшауен, горы Атлас",
        "🇯🇵 Япония — сакура! +18°C. Токио, Киото, Нара",
        "🇨🇾 Кипр — +23°C, тепло. Айя-Напа, Протарас",
        "🇪🇸 Испания — +22°C, весна. Севилья, Барселона, Мадрид",
        "🇮🇹 Италия — +20°C, цветение. Рим, Флоренция, Амальфи",
        "🇯🇴 Иордания — +24°C, лучший сезон. Петра, Мёртвое море",
        "🇹🇭 Таиланд — Сонгкран (Новый год), +35°C. Чиангмай",
        "🇻🇳 Вьетнам (юг) — +33°C. Хошимин, Фукуок, дельта Меконга",
        "🇱🇰 Шри-Ланка — +32°C. Нувара-Элия, чайные плантации",
        "🇹🇳 Тунис — +22°C. Карфаген, Матмата, оазисы",
        "🇸🇬 Сингапур — +31°C. Формула 1, ботанический сад, Марина Бэй",
        "🇲🇹 Мальта — +19°C, весна. Валлетта, Мдина, лазурное окно",
        "🇦🇪 ОАЭ — +30°C, последний комфортный месяц. Дубай",
    ],
    "Май": [
        "🇬🇷 Греция — +25°C, отлично. Крит, Родос, Корфу",
        "🇮🇹 Италия — +24°C, до жары. Амальфи, Сицилия, Тоскана",
        "🇪🇸 Испания — +25°C. Канарские острова, Андалусия",
        "🇹🇷 Турция — +28°C, открытие сезона. Анталья, Бодрум",
        "🇵🇹 Португалия — +22°C. Мадейра, Азорские острова",
        "🇭🇷 Хорватия — +23°C, тихо. Дубровник, Сплит, острова",
        "🇯🇵 Япония — +22°C, зелёный сезон. Хоккайдо, Никко",
        "🇮🇸 Исландия — +10°C, белые ночи начинаются. Рейкьявик",
        "🇲🇦 Марокко — +28°C, последний комфортный месяц",
        "🇦🇲 Армения — +22°C, цветение. Ереван, Гегард, Дилижан",
        "🇬🇪 Грузия — +24°C. Тбилиси, Казбеги, Кутаиси",
        "🇲🇪 Черногория — +22°C. Котор, Дурмитор, Будва",
        "🇺🇿 Узбекистан — +28°C. Самарканд, Бухара, Хива",
        "🇷🇸 Сербия — +22°C. Белград, Нови-Сад, монастыри Фрушка-горы",
        "🇰🇬 Киргизия — +20°C, весна. Иссык-Куль, ущелье Ала-Арча",
    ],
    "Июнь": [
        "🇮🇸 Исландия — белые ночи, +13°C. Полярный день, водопады",
        "🇳🇴 Норвегия — +18°C, фьорды. Берген, Гейрангер, Лофотены",
        "🇭🇷 Хорватия — +28°C, море. Дубровник, острова Хвар и Брач",
        "🇬🇷 Греция — +30°C, пляжный сезон. Миконос, Родос",
        "🇪🇸 Испания — +28°C. Балеарские острова, Коста-Брава",
        "🇵🇹 Португалия — +25°C. Алгарве, Лиссабон",
        "🇲🇪 Черногория — +28°C. Котор, Будва, Боко-Которский залив",
        "🇬🇪 Грузия — +28°C, сухо. Тбилиси, Кахетия, Сванетия",
        "🇦🇲 Армения — +28°C. Татев, озеро Севан, Хор Вирап",
        "🇦🇿 Азербайджан — +28°C. Баку, Гобустан, Габала",
        "🇸🇪 Швеция — +20°C, белые ночи. Стокгольм, острова, Мидсоммар",
        "🇫🇮 Финляндия — +20°C. Хельсинки, озёра, сауна на природе",
        "🇯🇵 Япония — +25°C. Хоккайдо — лаванда, Осака, гора Фудзи",
        "🇨🇭 Швейцария — +22°C. Люцерн, Женева, Церматт",
        "🇷🇸 Сербия — +26°C. Белград, Нови-Сад, Exit фестиваль",
    ],
    "Июль": [
        "🇮🇸 Исландия — +14°C, пик сезона. Кольцевая дорога, гейзеры",
        "🇳🇴 Норвегия — +20°C. Лофотены, Тромсё, Флом",
        "🇭🇷 Хорватия — +30°C. Острова Корчула, Млет, Шибеник",
        "🇲🇪 Черногория — +32°C. Пляжи, яхты, нацпарк Дурмитор",
        "🇬🇪 Грузия — +32°C. Батуми, горы Казбеги, Местиа",
        "🇦🇹 Австрия — +25°C. Зальцбург, Галльштат, Вена",
        "🇨🇭 Швейцария — +24°C. Интерлакен, Люцерн, Церматт",
        "🇵🇱 Польша — +24°C. Краков, Закопане, Вроцлав",
        "🇱🇹 Литва — +22°C. Вильнюс, Куршская коса, Тракай",
        "🇲🇳 Монголия — +25°C, фестиваль Надом. Степи, юрты",
        "🇸🇪 Швеция — +22°C. Гётеборг, Готланд, шхеры",
        "🇩🇰 Дания — +22°C. Копенгаген, Борнхольм, Фарерские острова",
        "🇯🇵 Япония — +30°C, фестивали. Киото Гион Мацури",
        "🇨🇦 Канада — +24°C. Ванкувер, Банф, Квебек",
        "🇷🇸 Сербия — +30°C. Белград, Тара, Дьявольский город",
    ],
    "Август": [
        "🇬🇪 Грузия — +32°C, пик. Батуми, Тбилиси, горные сёла",
        "🇭🇷 Хорватия — +32°C. Дубровник, острова, Плитвице",
        "🇮🇸 Исландия — +14°C. Последний месяц белых ночей",
        "🇲🇳 Монголия — +26°C. Гоби, конные маршруты, Хубсугул",
        "🇲🇪 Черногория — +33°C. Пляжный пик, Ада Бояна",
        "🇦🇱 Албания — +32°C, тихо. Саранда, Гирокастра, Ривьера",
        "🇲🇰 Северная Македония — +30°C. Охрид, монастыри",
        "🇱🇻 Латвия — +22°C. Рига, Юрмала, замки",
        "🇫🇮 Финляндия — +22°C. Хельсинки, озёра, сауна",
        "🇰🇿 Казахстан — +28°C. Алматы, Чарынский каньон, Кольсай",
        "🇸🇪 Швеция — +22°C. Лапландия, острова, фьорды",
        "🇨🇭 Швейцария — +24°C. Интерлакен, Гриндельвальд",
        "🇨🇦 Канада — +26°C. Банф, Джаспер, озеро Луиз",
        "🇷🇸 Сербия — +32°C. Нови-Сад, Exit, Суботица",
        "🇧🇬 Болгария — +30°C. Созополь, Несебыр, Бургас",
    ],
    "Сентябрь": [
        "🇬🇷 Греция — +26°C, бархатный сезон. Крит, Санторини",
        "🇮🇹 Италия — +24°C, идеально. Тоскана, Амальфи, Венеция",
        "🇪🇸 Испания — +26°C. Барселона, Мадрид, Сан-Себастьян",
        "🇵🇹 Португалия — +24°C. Алгарве, Лиссабон, Порту",
        "🇯🇵 Япония — +24°C, тихо до сезона. Киото, Осака",
        "🇲🇦 Марокко — +28°C, отлично. Марракеш, Атлас, Сахара",
        "🇬🇪 Грузия — +26°C, виноград. Кахетия, Тбилиси, Мцхета",
        "🇦🇲 Армения — +24°C. Нораванк, Татев, Хор Вирап",
        "🇺🇿 Узбекистан — +28°C, лучший месяц. Самарканд, Бухара",
        "🇹🇷 Турция — +28°C, бархатный сезон. Стамбул, Каппадокия",
        "🇦🇱 Албания — +26°C. Саранда, Тирана, Ривьера",
        "🇰🇿 Казахстан — +20°C. Алматы, Чарынский каньон, Астана",
        "🇦🇿 Азербайджан — +24°C. Баку, Шеки, Гянджа",
        "🇧🇬 Болгария — +24°C. Несебыр, Велико-Тырново, Рила",
        "🇷🇸 Сербия — +24°C. Белград, Zlatibor, монастыри",
    ],
    "Октябрь": [
        "🇯🇵 Япония — +18°C, золотая осень. Никко, Киото, Нара",
        "🇹🇷 Турция — +22°C, бархатный сезон. Стамбул, Каппадокия",
        "🇻🇳 Вьетнам (север) — +25°C, сухо. Ханой, Халонг, Хойан",
        "🇲🇦 Марокко — +25°C. Фес, Мерзуга, горы Атлас",
        "🇺🇿 Узбекистан — +22°C. Хива, Бухара, Самарканд",
        "🇮🇳 Индия (Раджастан) — +30°C, сухо. Джайпур, Джодхпур",
        "🇬🇷 Греция — +22°C, тихо. Афины, Крит, Родос",
        "🇪🇬 Египет — +28°C, открытие сезона. Хургада, Шарм, Луксор",
        "🇦🇿 Азербайджан — +20°C, осень. Баку, Шеки, Гянджа",
        "🇰🇬 Киргизия — +15°C. Иссык-Куль, ущелья, юрточные лагеря",
        "🇻🇳 Вьетнам (центр) — +28°C. Хойан, Дананг, Хюэ",
        "🇱🇰 Шри-Ланка (север) — +30°C. Джафна, Тринкомали",
        "🇰🇭 Камбоджа — +30°C, сухо. Ангкор-Ват, Пномпень",
        "🇲🇾 Малайзия — +30°C. Куала-Лумпур, Лангкави, Борнео",
        "🇮🇱 Израиль — +24°C. Тель-Авив, Иерусалим, Эйлат",
    ],
    "Ноябрь": [
        "🇻🇳 Вьетнам — +28°C, лучший сезон. Хойан, Дананг, Ханой",
        "🇹🇭 Таиланд — начало сезона, +30°C. Краби, Ко Самуи",
        "🇦🇪 ОАЭ — +28°C, комфортно. Дубай, Абу-Даби",
        "🇮🇳 Индия (Гоа) — +32°C, открытие. Северный Гоа, Панаджи",
        "🇪🇬 Египет — +26°C, пик сезона. Дайвинг в Красном море",
        "🇲🇻 Мальдивы — +30°C, сухой сезон начался",
        "🇿🇦 ЮАР — +25°C, весна. Кейптаун, сафари Крюгер",
        "🇴🇲 Оман — +28°C, отлично. Маскат, Низва, пустыня",
        "🇯🇴 Иордания — +22°C. Петра, Вади-Рам, Акаба",
        "🇰🇪 Кения — сухой сезон, сафари. Амбосели, Самбуру",
        "🇱🇰 Шри-Ланка (юг) — +30°C, начало пика. Мирисса, Галле",
        "🇰🇭 Камбоджа — +30°C. Ангкор-Ват после дождей — зелёный",
        "🇲🇾 Малайзия — +30°C. Пенанг, Малакка, острова Перхентиан",
        "🇮🇱 Израиль — +22°C. Иерусалим, Мёртвое море, Эйлат",
        "🇵🇭 Филиппины — +30°C, начало сезона. Палаван, Себу",
    ],
    "Декабрь": [
        "🇹🇭 Таиланд — +32°C, пик сезона. Пхукет, Ко Тао, Чиангмай",
        "🇦🇪 ОАЭ — +24°C, идеально. Дубай, Новый год с фейерверком",
        "🇲🇻 Мальдивы — +30°C. Лучшая видимость, рифы, скаты",
        "🇻🇳 Вьетнам — +28°C. Фукуок, Хошимин, Меконг",
        "🇮🇩 Бали — +29°C, сухо на юге. Семиньяк, Чангу, Убуд",
        "🇲🇽 Мексика — +28°C. Канкун, Тулум, Мерида",
        "🇰🇪 Кения — +28°C. Сафари Масаи-Мара, Момбаса",
        "🇪🇬 Египет — +22°C. Каир, пирамиды, круиз по Нилу",
        "🇴🇲 Оман — +26°C. Лучшее время — Маскат, вади, горы",
        "🇨🇺 Куба — +26°C. Гавана, Тринидад, Варадеро",
        "🇱🇰 Шри-Ланка (юг) — +31°C, пик. Унаватуна, киты у Мириссы",
        "🇵🇭 Филиппины — +30°C. Боракай, Палаван, Симала",
        "🇰🇭 Камбоджа — +28°C, лучший сезон. Ангкор на рассвете",
        "🇸🇬 Сингапур — +30°C. Новогодние огни Орчард-роуд",
        "🇮🇳 Индия (Гоа) — +32°C, пик. Пляжи, рынки, Новый год",
    ],
}

_WHERE_TO_FLY_MONTHS_KB = ReplyKeyboardMarkup(
    [
...
async def where_to_fly_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    if text == HOME_BTN:
        return await go_home(update, context)
    if text == "◀️ Назад":
    ...
    if not countries:
        return await where_to_fly_menu(update, context)
    context.user_data["where_to_fly_month"] = text
    lines = "\n".join(f"{i + 1}. {c}" for i, c in enumerate(countries))
    await update.message.reply_text(
        f"🗓 Куда лететь в {text}\n\n"
        f"Топ-15 направлений — сухой сезон, хорошая погода:\n\n"
        f"{lines}\n\n"
        "🌤 Данные основаны на климатических сезонах",
        reply_markup=ReplyKeyboardMarkup(
            [["◀️ Назад", HOME_BTN]],
            resize_keyboard=True,
        ),
    )
    return WHERE_TO_FLY_MONTH


async def where_to_fly_month_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    if text == HOME_BTN:
        return await go_home(update, context)
    return await where_to_fly_menu(update, context)
...
def waters_get_all(water_type: str) -> list[dict]:
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT id, name, ocean_id, countries, length_km, fun_fact FROM waters WHERE type=%s ORDER BY id",
                    (water_type,)
                )
                rows = cur.fetchall()
        finally:
            conn.close()
        return [{"id": r[0], "name": r[1], "ocean_id": r[2], "countries": r[3] or [], "length_km": r[4], "fun_fact": r[5]} for r in rows]
    except Exception as e:
        logger.error("waters_get_all: %s: %s", type(e).__name__, e)
        return []
...
def user_waters_get_ids(user_id: int) -> set[int]:
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT water_id FROM user_waters WHERE user_id=%s", (user_id,))
                rows = cur.fetchall()
        finally:
            conn.close()
        return {r[0] for r in rows}
    except Exception as e:
        logger.error("user_waters_get_ids: %s: %s", type(e).__name__, e)
        return set()


def user_waters_toggle(user_id: int, water_id: int) -> None:
    try:
        conn = get_db_connection()
        try:
            with conn.cursor() as cur:
                ...
                if cur.fetchone():
                    cur.execute("DELETE FROM user_waters WHERE user_id=%s AND water_id=%s", (user_id, water_id))
                else:
                    cur.execute("INSERT INTO user_waters (user_id, water_id) VALUES (%s, %s) ON CONFLICT DO NOTHING", (user_id, water_id))
            conn.commit()
        finally:
            conn.close()
    except Exception as e:
        logger.error("user_waters_toggle: %s: %s", type(e).__name__, e)
...
async def show_folder_mytrips(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data.clear()
    await update.message.reply_text(
        "🗺 *Мои путешествия*\n\nВыбери раздел:",
        parse_mode="Markdown",
...
async def waters_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text
    if text == HOME_BTN:
        return await go_home(update, context)
    if text == "◀️ Назад":
...
async def waters_ref_type_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text
    if text == HOME_BTN:
        return await go_home(update, context)
    if text == "◀️ Назад":
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            WEATHER_INPUT: [
                home,
                MessageHandler(filters.Regex("^◀️ Назад$"), show_folder_planning),
                MessageHandler(filters.TEXT & ~filters.COMMAND, weather_show),
            ],
            WATERS_MENU: [
                home,
                MessageHandler(filters.TEXT & ~filters.COMMAND, waters_menu_handler),
            ],
            WATERS_REF_TYPE: [
                home,
                MessageHandler(filters.TEXT & ~filters.COMMAND, waters_ref_type_handler),
            ],
            WATERS_REF_LIST: [
                home,
            ...
            WHERE_TO_FLY: [
                home,
                MessageHandler(filters.TEXT & ~filters.COMMAND, where_to_fly_handler),
            ],
            WHERE_TO_FLY_MONTH: [
                home,
                MessageHandler(filters.TEXT & ~filters.COMMAND, where_to_fly_month_handler),
            ],
        },
        fallbacks=[
            home,
